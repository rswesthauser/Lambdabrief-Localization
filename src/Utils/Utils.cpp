...
double Utils::Normalize(double value, double minRange, double maxRange, double minVal, double maxVal)
{
...
float Utils::RandomDouble(double begin, double end)
{
...
Pose3d::Pose3d()
{
    x=y=roll=pitch=yaw=0.0;
    z=1;
}

Pose3d::Pose3d(double x, double y, double z, double yaw)
{
    roll=pitch=0.0;
    this->x = x;
    this->y = y;
...
Pose3d::Pose3d(double x, double y, double z, double roll, double pitch, double yaw,  double yawInvertido)
{
    this->x = x;
    this->y = y;
    this->z = z;
    ...
    this->pitch = pitch;
    this->yaw = yaw;
    this->yawInvertido = yawInvertido;
}
...
LogFile::LogFile(LogMode mode, string name)
{
    ...
    if(mode == RECORDING)
    {
        ss << "../Sensors/sensors-" << -100+now->tm_year
...
void Utils::MatType( cv::Mat inputMat )
{
